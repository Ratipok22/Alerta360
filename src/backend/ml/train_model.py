...
def load_or_generate_dataset() -> pd.DataFrame:
    if DATA_PATH.exists():
        df = pd.read_csv(DATA_PATH, parse_dates=["fecha", "timestamp"])
        # Si el CSV en disco quedo de una version anterior sin variables
        # climaticas (ver ml/weather.py), se regenera en vez de fallar mas
        # adelante al buscar columnas que no existen.
        if set(FEATURE_COLUMNS_NUMERIC).issubset(df.columns):
            return df
    df = generate()
